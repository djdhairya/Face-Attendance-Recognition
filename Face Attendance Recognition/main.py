...
class App:
    def __init__(self):
        ...
        self.db_dir = 'C:/Users/Dhairya Hindoriya/OneDrive/Desktop/Face Attendance Recognition/db'
    ...
    def login(self):
        ...
        output = subprocess.check_output(['face_recognition', self.db_dir, tmp_path], text=True)
        name = output.split(',')[1][:-3].strip()

        if name in ['unknown_person', 'no_persons_found']:
            util.msg_box('Ups...', 'Unknown student. Please register a new user or try again.')
        else:
            util.msg_box('Welcome Back!', 'Welcome, {}.'.format(name))
            self.log_successful_login(name)

    def log_successful_login(self, username):
        try:
            with open(self.log_path, 'a') as f:
                f.write('{},{}\n'.format(username, datetime.datetime.now()))
            print(f"Successful login logged for {username}. Log file updated at: {os.path.abspath(self.log_path)}")
